...
pub fn nega_max(mut board: Board, depth: u8, color: i8, evaluator: fn(Board) -> f32) -> (f32, BitMove) {
    ...
    let mut best_score: f32 = -10000.0;
    let mut best_move: BitMove = BitMove::null();

    for mv in moves {
        board.apply_move(mv);
        let (mut score, _) = nega_max(board.shallow_clone(), depth - 1, -color, evaluator);
...
mod tests {
    ...
    fn take_queen_white(){
        ...
        let (_, mv) = nega_max(board, 4, 1, eval::eval);
    ...
    fn take_queen_black(){
        ...
        let (score, mv) = nega_max(board, 4, -1, eval::eval);
    ...
    fn mate_in_one_1(){
        ...
        for depth in 1..4 {
            let (_, mv) = nega_max(board.shallow_clone(), depth, 1, eval::eval);
            board.apply_move(mv);
    ...
    fn mate_in_two_1(){
        ...
        let mut color = 1;
        for _i in 0..3 {
            let (_, mv) = nega_max(board.shallow_clone(), 4, color, eval::eval);
            color = -color;
            board.apply_move(mv);
        }
